...
```bash
python lfs_wallpaper_generator.py
```

# Générer seulement les 5 premières en JPEG qualité 90, dans un dossier spécifique
```bash
python lfs_wallpaper_generator.py -o ./wallpapers -f jpg -q 90 -n 5
```

# Générer une version plus légère (800x600) pour aperçu
```bash
python lfs_wallpaper_generator.py --width 800 --height 600
```
# Utiliser une configuration externe (fichier JSON)
```bash
python lfs_wallpaper_generator.py --config-file mes_configs.json
```
